...
function gridCols(label, count) {
  const cols = [label]
  for (let i = 0; i < count; i++) cols.push('1fr')
  return cols.join(' ')
}
...
function CompareCell({ feature, company }) {
  if (feature === 'rating') {
    return (
      <span style={{ fontSize:11 }}>
        {ratingStr(company.rating)}
        <br />
        <span style={{ fontSize:9, color:'#6d7a74' }}>
  ...
  if (feature === 'pricing') {
    return <span style={{ fontSize:10, lineHeight:'1.4', display:'block' }}>{shortText(company.pricing, 30)}</span>
  }
  if (feature === 'website') {
    return (
      <a
...
function ComparePageInner() {
  ...
  function addToCompare(c) {
    ...
    setSelected(prev => {
      ...
      if (next.length >= 2) {
        setTimeout(() => {
          const el = document.getElementById('compare-table-main')
    ...
    <div style={{ minHeight:'100vh', background:'#f7f9fb', paddingTop:24 }}>
      ...
      {selected.length > 0 && (
        <div className="compare-mobile-bar" style={{
          display:'none', position:'sticky', top:54, zIndex:100,
          background:'#ffffff', borderBottom:'1px solid rgba(188,202,195,0.4)',
          padding:'10px 1rem', boxShadow:'0 2px 8px rgba(0,0,0,0.06)',
        }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, overflowX:'auto' }}>
            ...
            {selected.map(c => (
              <div key={c.id} style={{
                ...
                background:'rgba(0,132,137,0.08)', border:'1px solid rgba(0,132,137,0.2)',
                ...
                <button onClick={() => removeFromCompare(c.id)} style={{
                  background:'none', border:'none', cursor:'pointer', color:'#6d7a74', fontSize:14, lineHeight:1, padding:0,
                }}>x</button>
              </div>
            ))}
            {selected.length >= 2 && (
      ...
      <div style={{ maxWidth:1400, margin:'0 auto', padding:'2rem 2rem 1.5rem' }}>
        ...
        <h1 style={{ fontFamily:'Manrope,sans-serif', fontWeight:800,
          fontSize:'clamp(1.8rem,3.5vw,2.8rem)', letterSpacing:'-.03em', color:'#191c1e', marginBottom:8 }}>
      ...
      <div className="compare-grid" style={{
        maxWidth:1400, margin:'0 auto', padding:'0 2rem 4rem',
        display:'grid', gridTemplateColumns:'220px 1fr 260px', gap:24, alignItems:'start',
      }}>
        ...
        <div>
          ...
          {/* Compare table - shown above list when 2+ selected */}
          {selected.length >= 2 && (
            <div id="compare-table-main" style={{
              marginBottom:24, background:'#ffffff',
              border:'1px solid rgba(188,202,195,0.4)', borderRadius:16, overflow:'hidden',
              boxShadow:'0 4px 24px -4px rgba(0,0,0,0.07)',
            }}>
              <div style={{
                padding:'14px 20px', borderBottom:'1px solid rgba(188,202,195,0.3)',
                display:'flex', alignItems:'center', justifyContent:'space-between',
              }}>
                <div style={{ fontFamily:'Manrope,sans-serif', fontWeight:700, fontSize:14, color:'#191c1e' }}>
                  Comparing {selected.length} companies
                </div>
                <button onClick={() => setSelected([])} style={{
                  background:'none', border:'1px solid rgba(188,202,195,0.5)', borderRadius:6,
                  padding:'4px 10px', cursor:'pointer', fontSize:12, color:'#6d7a74', fontFamily:'Manrope,sans-serif',
                }}>
                  Clear all
                </button>
              </div>
              <div style={{ overflowX:'auto' }}>
                <div style={{
                  ...
                  borderBottom:'1px solid rgba(188,202,195,0.3)', minWidth:320,
                }}>
                  <div style={{ padding:'12px 14px', fontSize:11, fontWeight:700, color:'#6d7a74', textTransform:'uppercase', letterSpacing:'.06em' }}>
                    Feature
                  </div>
                  {selected.map(c => (
                    <div key={c.id} style={{ padding:'12px 10px', textAlign:'center', borderLeft:'1px solid rgba(188,202,195,0.3)' }}>
                      <div style={{ width:36, height:36, borderRadius:8, background:'#f0f2f5', margin:'0 auto 8px',
                        overflow:'hidden', border:'1px solid rgba(188,202,195,0.4)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                        <CompanyLogo website={c.website} name={c.name}/>
                      </div>
                      <div style={{ fontSize:12, fontWeight:700, color:'#191c1e', marginBottom:2, lineHeight:1.2 }}>{c.name}</div>
                      <div style={{ fontSize:11, color:'#d97706' }}>{ratingStr(c.rating)}</div>
                    </div>
                  ))}
                </div>
                {FEATURES.map((f, fi) => (
                  <div key={f.key} style={{
                    ...
                    <div style={{ padding:'10px 14px', fontSize:11, fontWeight:600, color:'#6d7a74',
                      textTransform:'uppercase', letterSpacing:'.04em', alignSelf:'center' }}>
                      {f.label}
                    </div>
                    {selected.map(c => (
                      <div key={c.id} style={{ padding:'10px 8px', fontSize:11, color:'#191c1e',
                        textAlign:'center', borderLeft:'1px solid rgba(188,202,195,0.15)', alignSelf:'center', wordBreak:'break-word' }}>
                ...
                <div style={{
                  display:'grid',
                  gridTemplateColumns: gridCols('130px', selected.length),
                  borderTop:'1px solid rgba(188,202,195,0.3)', minWidth:320,
                }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
            <div style={{
              ...
              border:'1px solid rgba(188,202,195,0.5)', borderRadius:8, padding:'0 14px',
          ...
          {notFound && search && (
            <div style={{ padding:'14px 16px', borderRadius:10, marginBottom:12,
              ...
              <div>
                <div style={{ fontFamily:'Manrope,sans-serif', fontWeight:600, fontSize:14, color:'#191c1e', marginBottom:2 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {loading ? (
              ...
              const tagColor = catColors[c.category] || '#6d7a74'
              return (
                <div key={c.id}
                  style={{ background:'#ffffff', border:'1px solid rgba(188,202,195,0.4)',
                    borderRadius:12, padding:'1rem 1.25rem', boxShadow:'0 1px 3px rgba(0,0,0,0.04)',
                    transition:'box-shadow .2s, border-color .2s, transform .2s' }}
                  onMouseEnter={e => {
                    e.currentTarget.style.boxShadow = '0 4px 16px rgba(0,0,0,0.09)'
                    e.currentTarget.style.borderColor = 'rgba(0,132,137,0.2)'
                ...
                >
                  <div style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
                    <div style={{ width:44, height:44, flexShrink:0, borderRadius:8, background:'#f0f2f5',
                      border:'1px solid rgba(188,202,195,0.4)', display:'flex', alignItems:'center', justifyContent:'center',
                      overflow:'hidden' }}>
                      <CompanyLogo website={c.website} name={c.name}/>
                    </div>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:8, marginBottom:4 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                          <span style={{ fontFamily:'Manrope,sans-serif', fontWeight:700, fontSize:15, color:'#191c1e' }}>
                            {c.name}
                          </span>
                          {c.badge && (
                            <span style={{ fontSize:9, fontWeight:700, letterSpacing:'.08em', padding:'2px 7px', borderRadius:3,
                              textTransform:'uppercase', background:'rgba(37,99,235,0.1)', color:'#1d4ed8',
                              border:'1px solid rgba(37,99,235,0.2)' }}>
                              {c.badge}
                            </span>
                          )}
                          <span style={{ fontSize:10, fontWeight:500, padding:'2px 8px', borderRadius:3,
                            background: `${tagColor}20`, color: tagColor }}>
                            {c.category}
                          </span>
                        </div>
                        <div style={{ flexShrink:0, textAlign:'right' }}>
                          <div style={{ color:'#d97706', fontSize:11 }}>{ratingStr(c.rating)}</div>
                          <div style={{ fontSize:11, color:'#6d7a74', marginTop:1 }}>
                            {(c.review_count || 0).toLocaleString()} reviews
                          </div>
                        </div>
                      </div>
                      ...
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
                        flexWrap:'wrap', gap:8, paddingTop:10, borderTop:'1px solid rgba(188,202,195,0.3)' }}>
                        ...
                        <div style={{ display:'flex', gap:8 }}>
                          <button onClick={() => addToCompare(c)} style={{
                            ...
                            border: isSelected ? '1px solid #008489' : '1px solid rgba(188,202,195,0.5)',
                            background: isSelected ? 'rgba(0,132,137,0.08)' : 'transparent',
                            color: isSelected ? '#008489' : '#6d7a74',
                          ...
                          <a href={c.affiliate_url || c.website} target="_blank" rel="noopener noreferrer"
                            style={{ fontSize:11, padding:'5px 12px', borderRadius:6, background:'#008489',
                              color:'#ffffff', textDecoration:'none', fontFamily:'Manrope,sans-serif', fontWeight:500 }}>
                            Visit site
                          </a>
                        </div>
                      </div>
          ...
          {visible < filtered.length && (
            <div style={{ textAlign:'center', marginTop:24 }}>
              <button onClick={() => setVisible(v => v + 20)} style={{
                padding:'10px 28px', borderRadius:8, border:'1px solid rgba(188,202,195,0.5)',
                background:'#ffffff', color:'#3d4945', fontFamily:'Manrope,sans-serif',
        ...
        {/* RIGHT: Desktop compare panel */}
        <div style={{ position:'sticky', top:80 }} className="compare-panel-desktop">
          <div style={{ background:'#ffffff', border:'1px solid rgba(188,202,195,0.4)',
            borderRadius:16, padding:'1.25rem', boxShadow:'0 4px 24px -4px rgba(0,0,0,0.07)' }}>
            <div style={{ fontFamily:'Manrope,sans-serif', fontWeight:700, fontSize:14, color:'#191c1e', marginBottom:4 }}>
              Compare companies
            </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:12 }}>
              {[0, 1, 2].map(i => (
                <div key={i} style={{
                  ...
                  border: selected[i] ? '1px solid rgba(0,132,137,0.25)' : '1px dashed rgba(188,202,195,0.5)',
                  ...
                  {selected[i] ? (
                    <>
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        ...
                        <span style={{ fontSize:12, fontWeight:500, color:'#008489' }}>{selected[i].name}</span>
                      </div>
                      <button onClick={() => removeFromCompare(selected[i].id)} style={{
                        background:'none', border:'none', cursor:'pointer', color:'#6d7a74', fontSize:16, lineHeight:1, padding:'0 2px',
                      }}>x</button>
                    </>
                  ) : (
              ...
              <div style={{ position:'relative', marginBottom:12 }}>
                <input value={compareSearch} onChange={e => setCompareSearch(e.target.value)}
                  ...
                  style={{ width:'100%', padding:'8px 12px', borderRadius:7,
                    border:'1px solid rgba(188,202,195,0.5)', fontSize:12, fontFamily:'Manrope,sans-serif',
                    background:'#f7f9fb', outline:'none', color:'#191c1e', boxSizing:'border-box' }}/>
                {compareSearchResults.length > 0 && (
                  <div style={{ position:'absolute', top:'calc(100% + 4px)', left:0, right:0, background:'#ffffff',
                    border:'1px solid rgba(188,202,195,0.5)', borderRadius:8, overflow:'hidden', zIndex:50,
                    boxShadow:'0 8px 24px rgba(0,0,0,0.1)' }}>
                    {compareSearchResults.map(c => (
                      <div key={c.id} onClick={() => { addToCompare(c); setCompareSearch('') }}
                        style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 12px', cursor:'pointer',
                          borderBottom:'1px solid rgba(188,202,195,0.3)', transition:'background .12s' }}
            ...
            {selected.length >= 2 ? (
              ...
              <div style={{ padding:'10px', borderRadius:8, background:'#f0f2f5', color:'#6d7a74',
                textAlign:'center', fontSize:13, fontFamily:'Manrope,sans-serif' }}>
          ...
          {selected.length >= 2 && (
            <div style={{ marginTop:16, background:'#ffffff', border:'1px solid rgba(188,202,195,0.4)',
              borderRadius:16, overflow:'hidden', boxShadow:'0 4px 24px -4px rgba(0,0,0,0.07)' }}>
              <div style={{ overflowX:'auto' }}>
                <div style={{
                  display:'grid', gridTemplateColumns: gridCols('100px', selected.length),
                  borderBottom:'1px solid rgba(188,202,195,0.3)', minWidth:260,
                }}>
                  <div style={{ padding:'10px 8px' }}/>
                  {selected.map(c => (
                    <div key={c.id} style={{ padding:'10px 6px', textAlign:'center', borderLeft:'1px solid rgba(188,202,195,0.3)' }}>
                      <div style={{ width:28, height:28, borderRadius:6, background:'#f0f2f5', margin:'0 auto 6px',
                        overflow:'hidden', border:'1px solid rgba(188,202,195,0.4)',
                        display:'flex', alignItems:'center', justifyContent:'center' }}>
                        <CompanyLogo website={c.website} name={c.name}/>
                      </div>
                      <div style={{ fontSize:10, fontWeight:700, color:'#191c1e', marginBottom:2, lineHeight:1.2, wordBreak:'break-word' }}>
                        {c.name}
                      </div>
                      <div style={{ color:'#d97706', fontSize:9 }}>{c.rating}</div>
                    </div>
                  ))}
                </div>
                {FEATURES.map((f, fi) => (
                  <div key={f.key} style={{
                    ...
                    <div style={{ padding:'7px 8px', fontSize:10, fontWeight:600, color:'#6d7a74',
                      textTransform:'uppercase', letterSpacing:'.03em', alignSelf:'center', lineHeight:1.3 }}>
                      {f.label}
                    </div>
                    {selected.map(c => (
                      <div key={c.id} style={{ padding:'7px 6px', fontSize:11, color:'#191c1e',
                        textAlign:'center', borderLeft:'1px solid rgba(188,202,195,0.2)',
                        alignSelf:'center', wordBreak:'break-word', lineHeight:1.4 }}>
                        <CompareCell feature={f.key} company={c}/>
                      </div>
                    ))}
                  </div>
