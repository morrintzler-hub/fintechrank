...
// Pure JS helpers - no JSX
function gridCols(n) {
  let s = '130px'
  for (let i = 0; i < n; i++) s += ' 1fr'
  return s
}

function gridColsSmall(n) {
  let s = '100px'
  for (let i = 0; i < n; i++) s += ' 1fr'
  return s
}
...
function shortDomain(url) {
  if (!url) return '-'
  return url.replace('https://','').replace('http://','').replace('www.','').split('/')[0].slice(0,18)
}
...
function Logo({ website, name }) {
  ...
  return (
    <span style={{ fontFamily:'Manrope,sans-serif', fontWeight:600, fontSize:11, color:'#6d7a74' }}>
    ...
    return (
      <span style={{ fontSize:11 }}>
        ...
        <span style={{ fontSize:9, color:'#94a3b8' }}>
          {(co.review_count || 0).toLocaleString()}
        </span>
      </span>
    )
  }
  if (field === 'pricing') {
    return <span style={{ fontSize:10, lineHeight:'1.4', display:'block' }}>{shortText(co.pricing, 28)}</span>
  }
  return <span style={{ fontSize:11 }}>{co[field] || '-'}</span>
}

// Slot component for the right-panel slots
function Slot({ co, idx, onRemove }) {
  const filled = !!co
  const borderStyle = filled
    ? '1px solid rgba(0,132,137,0.25)'
    : '1px dashed rgba(188,202,195,0.5)'
  return (
    <div style={{
      display:'flex', alignItems:'center', justifyContent:'space-between',
      padding:'7px 10px', borderRadius:8, minHeight:34,
      border: borderStyle,
      background: filled ? 'rgba(0,132,137,0.04)' : 'transparent',
      transition:'all .2s',
    }}>
      {filled ? (
        <>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <div style={{
              width:24, height:24, borderRadius:5, background:'#f0f2f5',
              border:'1px solid rgba(188,202,195,0.4)', overflow:'hidden',
              display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0,
            }}>
              <Logo website={co.website} name={co.name} />
            </div>
            <span style={{ fontSize:12, fontWeight:500, color:'#008489' }}>{co.name}</span>
          </div>
          <button
            onClick={() => onRemove(co.id)}
            style={{ background:'none', border:'none', cursor:'pointer', color:'#6d7a74', fontSize:16, lineHeight:1, padding:'0 2px' }}
          >
            x
          </button>
        </>
      ) : (
        <span style={{ fontSize:12, color:'#6d7a74' }}>+ Company {idx + 1}</span>
      )}
    </div>
  )
}

function CompareTable({ selected, size }) {
  const isSmall = size === 'small'
  const gcols = isSmall ? gridCols(selected.length) : gridColsSmall(selected.length)
  const minW = isSmall ? 320 : 260
  const labelPad = isSmall ? '10px 14px' : '7px 8px'
  const cellPad = isSmall ? '10px 8px' : '7px 6px'
  const labelSize = isSmall ? 11 : 10
  const cellSize = isSmall ? 11 : 10
  const logoSize = isSmall ? 36 : 28
  const nameSize = isSmall ? 12 : 10

  return (
    <div style={{ overflowX:'auto' }}>
      <div style={{
        display:'grid', gridTemplateColumns: gcols,
        borderBottom:'1px solid rgba(188,202,195,0.3)', minWidth: minW,
      }}>
        <div style={{ padding: labelPad, fontSize: labelSize, fontWeight:700, color:'#6d7a74', textTransform:'uppercase', letterSpacing:'.06em' }}>
          Feature
        </div>
        {selected.map(co => (
          <div key={co.id} style={{ padding:'10px 8px', textAlign:'center', borderLeft:'1px solid rgba(188,202,195,0.3)' }}>
            <div style={{
              width: logoSize, height: logoSize, borderRadius:8, background:'#f0f2f5',
              margin:'0 auto 6px', overflow:'hidden', border:'1px solid rgba(188,202,195,0.4)',
              display:'flex', alignItems:'center', justifyContent:'center',
            }}>
              <Logo website={co.website} name={co.name} />
            </div>
            <div style={{ fontSize: nameSize, fontWeight:700, color:'#191c1e', marginBottom:2, lineHeight:1.2 }}>
              {co.name}
            </div>
      ...
      {FIELDS.map((f, fi) => (
        <div key={f.key} style={{
          display:'grid', gridTemplateColumns: gcols,
          borderBottom: fi < FIELDS.length - 1 ? '1px solid rgba(188,202,195,0.15)' : 'none',
          background: fi % 2 === 0 ? 'transparent' : 'rgba(247,249,251,0.6)',
          minWidth: minW,
        }}>
          <div style={{ padding: labelPad, fontSize: labelSize, fontWeight:600, color:'#6d7a74', textTransform:'uppercase', letterSpacing:'.04em', alignSelf:'center' }}>
            {f.label}
          </div>
          {selected.map(co => (
            <div key={co.id} style={{ padding: cellPad, fontSize: cellSize, color:'#191c1e', textAlign:'center', borderLeft:'1px solid rgba(188,202,195,0.15)', alignSelf:'center', wordBreak:'break-word', lineHeight:1.4 }}>
              <Cell field={f.key} co={co} />
            </div>
          ))}
        </div>
      ))}
      <div style={{ display:'grid', gridTemplateColumns: gcols, borderTop:'1px solid rgba(188,202,195,0.3)', minWidth: minW }}>
        <div style={{ padding: labelPad }} />
        {selected.map(co => (
          <div key={co.id} style={{ padding:'10px 8px', textAlign:'center', borderLeft:'1px solid rgba(188,202,195,0.2)' }}>
            <a
              href={'/review/' + co.slug}
              style={{ display:'inline-block', padding:'6px 14px', borderRadius:6, background:'#008489', color:'white', textDecoration:'none', fontSize:11, fontWeight:600, fontFamily:'Manrope,sans-serif' }}
            >
              Full review
            </a>
          </div>
...
function Inner() {
  ...
  function remSel(id) {
    setSel(prev => prev.filter(c => c.id !== id))
    setPreset(null)
  }
  ...
  const cresults = csearch.trim()
    ? all.filter(c => c.name.toLowerCase().includes(csearch.toLowerCase()) && !sel.find(s => s.id === c.id)).slice(0,6)
    : []
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f7f9fb', paddingTop:24 }}>
      ...
      {sel.length > 0 && (
        <div className="compare-mobile-bar" style={{ display:'none', position:'sticky', top:54, zIndex:100, background:'#fff', borderBottom:'1px solid rgba(188,202,195,0.4)', padding:'10px 1rem', boxShadow:'0 2px 8px rgba(0,0,0,0.06)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, overflowX:'auto' }}>
            ...
            {sel.length >= 2 && (
              <a href={'/compare?ids=' + sel.map(c => c.slug).join(',')} style={{ padding:'5px 12px', borderRadius:6, background:'#008489', color:'white', textDecoration:'none', fontSize:11, fontWeight:600, whiteSpace:'nowrap', flexShrink:0, fontFamily:'Manrope,sans-serif' }}>Compare</a>
            )}
          </div>
        </div>
      )}
      ...
      <div style={{ maxWidth:1400, margin:'0 auto', padding:'2rem 2rem 1.5rem' }}>
        <h1 style={{ fontFamily:'Manrope,sans-serif', fontWeight:800, fontSize:'clamp(1.8rem,3.5vw,2.8rem)', letterSpacing:'-.03em', color:'#191c1e', marginBottom:8 }}>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:'1.5rem' }}>
          ...
          {POPULAR.map(p => (
            <button
              ...
              style={{ fontSize:12, padding:'5px 12px', borderRadius:6, cursor:'pointer', fontFamily:'Manrope,sans-serif', transition:'all .15s', border: preset === p.label ? '1px solid #008489' : '1px solid rgba(188,202,195,0.5)', background: preset === p.label ? 'rgba(0,132,137,0.08)' : '#fff', color: preset === p.label ? '#008489' : '#3d4945' }}
            >
              {p.label}
            </button>
          ))}
            ...
            {CATS.map(c => (
              ...
              >
                ...
                <span style={{ fontSize:11, color:'#6d7a74', background:'rgba(0,0,0,0.04)', padding:'1px 7px', borderRadius:3 }}>
                  {c.key === 'all' ? all.length : (counts[c.key] || 0)}
                </span>
              </button>
            ))}
          </div>
          <div>
            ...
            {SORTS.map(o => (
              <button
                ...
                style={{ display:'flex', alignItems:'center', gap:8, width:'100%', padding:'7px 10px', borderRadius:6, border:'none', background:'transparent', cursor:'pointer', fontFamily:'Manrope,sans-serif', marginBottom:2 }}
              >
        ...
        <div>
          {sel.length >= 2 && (
            <div id="ctable" style={{ marginBottom:24, background:'#fff', border:'1px solid rgba(188,202,195,0.4)', borderRadius:16, overflow:'hidden', boxShadow:'0 4px 24px -4px rgba(0,0,0,0.07)' }}>
              ...
              <CompareTable selected={sel} size="large" />
            </div>
          )}
          ...
          {notFound && search && (
            <div style={{ padding:'14px 16px', borderRadius:10, marginBottom:12, background:'rgba(220,38,38,0.04)', border:'1px solid rgba(220,38,38,0.15)', display:'flex', gap:12 }}>
              <div>
                ...
                <div style={{ fontFamily:'Manrope,sans-serif', fontSize:13, color:'#6d7a74' }}>
                  ...
                  <a href={'mailto:hello@thefintechrank.com?subject=Missing: ' + search} style={{ color:'#008489', textDecoration:'none', fontWeight:500 }}>Suggest a company</a>
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {loading ? (
              ...
              return (
                <div
                  key={co.id}
                  style={{ background:'#fff', border:'1px solid rgba(188,202,195,0.4)', borderRadius:12, padding:'1rem 1.25rem', boxShadow:'0 1px 3px rgba(0,0,0,0.04)', transition:'box-shadow .2s, border-color .2s, transform .2s' }}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:8, marginBottom:4 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                          ...
                          {co.badge && (
                            <span style={{ fontSize:9, fontWeight:700, letterSpacing:'.08em', padding:'2px 7px', borderRadius:3, textTransform:'uppercase', background:'rgba(37,99,235,0.1)', color:'#1d4ed8', border:'1px solid rgba(37,99,235,0.2)' }}>{co.badge}</span>
                          )}
                      ...
                      <p style={{ fontSize:13, color:'#3d4945', lineHeight:1.65, marginBottom:12, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>
                        {co.description}
                      </p>
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8, paddingTop:10, borderTop:'1px solid rgba(188,202,195,0.3)' }}>
                        <div style={{ fontSize:12, color:'#3d4945' }}>
                          <strong style={{ fontWeight:500, color:'#191c1e' }}>Pricing: </strong>{co.pricing}
                        </div>
                        <div style={{ display:'flex', gap:8 }}>
                          <button
                            onClick={() => addSel(co)}
                            style={{ fontSize:11, padding:'5px 12px', borderRadius:6, cursor:'pointer', fontFamily:'Manrope,sans-serif', fontWeight:500, transition:'all .15s', border: isSel ? '1px solid #008489' : '1px solid rgba(188,202,195,0.5)', background: isSel ? 'rgba(0,132,137,0.08)' : 'transparent', color: isSel ? '#008489' : '#6d7a74' }}
                          >
                            {isSel ? 'Added' : '+ Compare'}
                          </button>
          ...
          {visible < filtered.length && (
            <div style={{ textAlign:'center', marginTop:24 }}>
              <button onClick={() => setVisible(v => v + 20)} style={{ padding:'10px 28px', borderRadius:8, border:'1px solid rgba(188,202,195,0.5)', background:'#fff', color:'#3d4945', fontFamily:'Manrope,sans-serif', fontSize:14, cursor:'pointer' }}>
        ...
        <div style={{ position:'sticky', top:80 }} className="compare-panel-desktop">
          <div style={{ background:'#fff', border:'1px solid rgba(188,202,195,0.4)', borderRadius:16, padding:'1.25rem', boxShadow:'0 4px 24px -4px rgba(0,0,0,0.07)' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:12 }}>
              <Slot co={sel[0]} idx={0} onRemove={remSel} />
              <Slot co={sel[1]} idx={1} onRemove={remSel} />
              <Slot co={sel[2]} idx={2} onRemove={remSel} />
            </div>
            {sel.length < 3 && (
              <div style={{ position:'relative', marginBottom:12 }}>
                <input
                  ...
                  style={{ width:'100%', padding:'8px 12px', borderRadius:7, border:'1px solid rgba(188,202,195,0.5)', fontSize:12, fontFamily:'Manrope,sans-serif', background:'#f7f9fb', outline:'none', color:'#191c1e', boxSizing:'border-box' }}
                />
                {cresults.length > 0 && (
                  <div style={{ position:'absolute', top:'calc(100% + 4px)', left:0, right:0, background:'#fff', border:'1px solid rgba(188,202,195,0.5)', borderRadius:8, overflow:'hidden', zIndex:50, boxShadow:'0 8px 24px rgba(0,0,0,0.1)' }}>
                    {cresults.map(co => (
                      <div
                        key={co.id}
                        onClick={() => { addSel(co); setCsearch('') }}
                        style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 12px', cursor:'pointer', borderBottom:'1px solid rgba(188,202,195,0.3)', transition:'background .12s' }}
            ...
            {sel.length >= 2 ? (
              <a
                href={'/compare?ids=' + sel.map(c => c.slug).join(',')}
                style={{ display:'block', width:'100%', padding:'10px', borderRadius:8, background:'#008489', color:'#fff', textAlign:'center', textDecoration:'none', fontSize:13, fontWeight:600, fontFamily:'Manrope,sans-serif' }}
              >
                Compare now
              </a>
            ) : (
              <div style={{ padding:'10px', borderRadius:8, background:'#f0f2f5', color:'#6d7a74', textAlign:'center', fontSize:13, fontFamily:'Manrope,sans-serif' }}>
                {sel.length === 0 ? 'Select 2-3 companies' : 'Add 1 more company'}
              </div>
            )}
          </div>
          {sel.length >= 2 && (
            <div style={{ marginTop:16, background:'#fff', border:'1px solid rgba(188,202,195,0.4)', borderRadius:16, overflow:'hidden', boxShadow:'0 4px 24px -4px rgba(0,0,0,0.07)' }}>
              <CompareTable selected={sel} size="small" />
            </div>
          )}
        </div>
      </div>
